...
differential equations with an adaptive Runge-Kutta scheme. The GYM assumes the number \(N_{a,y}\),
biomass \(B_{a,y}\) and yield \(Y_{a,y}\) of individuals of age \(a\) in year \(y\) satisfy the
system of differential equations (@GYM96)
\[
\begin{align}
...
The precise form of the functional relationships between these quantities is essentially arbitrary
...
single year, \(M_{y}\) and \(F_{y}\) are just constants, and the intra-annual components are stored
as annual matrices:
...
\]
The `trapz` function implements the composite trapezoidal rule.  Given a vector `fs` of function
...
be scaled to reproduce both the relative abundance and the known total biomass.  In essence this
...
* `yield` - if `yield=1` only calculate yield for the final time step, if `yield=2`
  calculate yield for all time steps, otherwise yield is not calculated at all
...
Given the assumptions of the model, the computation of `N` and `B` are exact, but the trapezoidal
...
The `projectC` function harnesses `project` in conjunction with the univariate root finder `uniroot` to
...
Grym requires matrices that map age at each time step to weight, maturity or selectivity.  In
...
Length-weight relationships are often modelled as a power law
```{r,eval=FALSE}
<<powerLW>>
```
...
Grym also provides a logistic alternative parameterized in terms of the 50th and 95th percentiles
```{r,eval=FALSE}
<<logisticOgive>>
```

and a double Normal selectivity parameterized in terms of the endpoints, widths and minimia of the
...
many years as there are age classes with non negligible numbers (including age classes
...
### Deterministic Virgin Spawning Biomass
...
### Stochastic Virgin Spawning Biomass
...
\[
\begin{align}
\mu &= \log \frac{\mu_{Q}^{2}}{\sqrt{\mu_{Q}^{2} + \sigma_{Q}^{2}}}\\
...
The `surveySurvival` function determines the total survival from recruitment to a nominated survey
...
so that `yr==1` corresponds to the year of the first element of `M` or `F`.  Survival cannot be
...
* `Fs` - matrix of unscaled integrated fishing mortalities 
* `F` - vector of annual fishing mortalities
* `rcls` the reference age class to adjust to
...
The `surveyAdjustGYM` function adjusts surveyed age class abundances
to an initial abundance at a reference age class following the
procedure implemented in the GYM.

In particular, where recruitment is estimated from multiple surveys
the GYM averages estimates from independent surveys by taking
geometric means weighting by the inverse of the squared *coefficient
of variation*.

Given 

* `survey.df` a dataframe of surveyed abundance (see details)
* `Ms` matrix of unscaled integrated natural mortality
* `M` vector of annual natural mortalities
* `rcls` the reference age class to adjust to
...
Estimates are generated for a contiguous sequence of years from the
earliest year to the latest year for which the abundance of the
reference class can be estimated, with `NA` returned for years for
which no estimate can be made.
```{r,eval=FALSE}
<<surveyAdjustGYM>>
```
...
similar functions can expect subtly different arguments, which is likely to lead to programming
...
The GYM standardizes the natural and fishing mortalities according to specific rules, but it is not
clear that these rules are consistent if the fishing season is allowed to change inter-annually.  In
Grym, we have not formalized the standardization in any way, we have left it up to the programmer
...
implementation seems oddly inaccurate.  For example - when calculating median SB0, the GYM projects 
...
can be distinguished by whether the `plus` argument is Boolean (infinite age) or integer (finite age).
...
But the process for resampling the mean and variance of \(R\) proposed in @DLMrecruit94 is not
...
summary to the appropriate row of the dataframe, R performs an unnecessary copy of the data frame
...
vastly reduced.  There is a helpful discussion on this [blog](https://www.brodieg.com/2019/02/18/an-unofficial-reference-for-internal-inspect/)
...
* store results to vectors that are then bound together to form a dataframe rather than allocate
  dataframes and write individual results to that dataframe, and
